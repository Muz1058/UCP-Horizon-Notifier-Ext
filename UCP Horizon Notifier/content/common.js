...
(function (global) {
    ...
    chrome.storage.local.get([storageKey, "hz_read_ids"], res => {
      ...
      const toMark = items
        .filter(item => {
          ...
          if (item.courseId)    return item.courseId === courseId;  // exact courseId match
          return cleanName(item.courseName||"") === cn; // fallback: exact name match
        })
        .map(item => item.id);
  ...
  // All four tab types auto-mark on visit — the correct storage key is used per tab.
  // Delay 3s: gives the page scanner (1.5s) and background scan time to write first.
  if (["announcement","outline","material","submission","grade"].includes(tab)) {
    setTimeout(markCurrentTabAsRead, 3000);
  }
